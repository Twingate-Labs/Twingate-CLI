...
def get_set_allow_resources(token,domains):
    Headers = StdAPIUtils.get_api_call_headers(token)
...
def get_set_deny_resources(token,domains):
    Headers = StdAPIUtils.get_api_call_headers(token)
...
def item_show(outputFormat,sessionname):
...
def set_allow_list(outputFormat,sessionname,domains):
    print(domains)
    j = StdAPIUtils.generic_api_call_handler(sessionname,get_set_allow_resources,domains)
    output,r = StdAPIUtils.format_output(j,outputFormat,DNSSecTransformers.GetUpdateAllowAsCsv)
    print(output)

def set_deny_list(outputFormat,sessionname,domains):
    j = StdAPIUtils.generic_api_call_handler(sessionname,get_set_deny_resources,domains)
    output,r = StdAPIUtils.format_output(j,outputFormat,DNSSecTransformers.GetUpdateDenyAsCsv)
    print(output)
